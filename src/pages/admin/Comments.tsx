...
const Comments: React.FC = () => {
  ...
  const handleOk = async () => {
    ...
    await updateComment(editing.id, values);
  ...
  const columns = [
    ...
    { title: 'Trạng thái', dataIndex: 'status', key: 'status', render: (status: string, item: any) => <Tag color={status === 'visible' ? 'green' : 'gray'} onClick={() => handleToggleStatus(item)} style={{ cursor: 'pointer' }}>{status === 'visible' ? 'Hiển thị' : 'Đã ẩn'}</Tag> },
    { title: 'Ngày tạo', dataIndex: 'createdAt', key: 'createdAt', render: (d: string) => dayjs(d).format('DD/MM/YYYY') },
    {
      title: 'Hành động', key: 'action', render: (_: any, item: any) => (
        <Space>
          ...
          <Popconfirm title="Xoá bình luận này?" onConfirm={() => handleDelete(item.id)}>
