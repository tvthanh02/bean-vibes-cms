...
import { getReviews, approveReview, rejectReview, updateReview, deleteReview, toggleVisibleReview } from '../../api/moderatorReviews';
...
const Reviews: React.FC = () => {
  ...
  const [filter, setFilter] = useState({ title: '', submittedBy: '', status: '', visible: '', dateFrom: '', dateTo: '' });
  ...
  const handleToggleVisible = async (item: any) => {
    await toggleVisibleReview(item.id);
    message.success(item.visible ? 'Đã ẩn bài viết!' : 'Đã hiển thị bài viết!');
    fetchData();
  };
  ...
  const columns = [
    ...
    { title: 'Tiêu đề', dataIndex: 'title', key: 'title', render: (text: string) => <Tooltip title={text}>{text.length > 30 ? text.slice(0, 30) + '...' : text}</Tooltip> },
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        <Input placeholder="Tiêu đề" value={filter.title} onChange={e => setFilter(f => ({ ...f, title: e.target.value }))} style={{ width: 160 }} allowClear />
      ...
      >
        {detail && (
          <div className="space-y-2">
            ...
            <div><b>Tiêu đề:</b> {detail.title}</div>
